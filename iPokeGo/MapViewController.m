...
-(void)initObserver
{
	...
	UILongPressGestureRecognizer *mapLongPressGesture = [[UILongPressGestureRecognizer alloc] initWithTarget:self action:@selector(handleMapLongPress:)];
    [self.mapview addGestureRecognizer:mapLongPressGesture];
	
	UILongPressGestureRecognizer *locationButtonLongPressGesture = [[UILongPressGestureRecognizer alloc] initWithTarget:self action:@selector(handleLocationButtonLongPress:)];
	[self.locationButton addGestureRecognizer:locationButtonLongPressGesture];
...
-(BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldRecognizeSimultaneouslyWithGestureRecognizer:(UIGestureRecognizer *)otherGestureRecognizer {
	if([gestureRecognizer class] == [otherGestureRecognizer class]) {
		// Otherwise mapPanGesture will be recognized together with every other recognizer, instead of just the actual pan gesturerecognizer MKMapView uses.
		return YES;
	} else {
		return NO;
	}
...
-(void)handleLocationButtonLongPress:(UIGestureRecognizer*)sender {
	if (sender.state == UIGestureRecognizerStateBegan) {
		AudioServicesPlayAlertSound(kSystemSoundID_Vibrate);
		...
		if(!isTrackingLocation) {
			...
			[_locationButton.layer addAnimation:pulseAnimation forKey:nil];
...
-(void)loadSavedData {
	...
	if([defaults objectForKey:@"lured_only"] != nil) {
		showLureOnly = [defaults boolForKey:@"lured_only"];
	}
...
- (void)mapView:(MKMapView *)mapView regionDidChangeAnimated:(BOOL)animated {
    ...
    for (int i = 0; i < [annotations count]; i++) {
        ...
        if (mapView.region.span.latitudeDelta > .20) {
    ...
    NSUserDefaults *prefs = [NSUserDefaults standardUserDefaults];
    NSDictionary *mapRegion = [[NSDictionary alloc] initWithObjects:@[[NSNumber numberWithDouble:mapView.region.center.latitude], [NSNumber numberWithDouble:mapView.region.center.longitude], [NSNumber numberWithDouble:mapView.region.span.latitudeDelta], [NSNumber numberWithDouble:mapView.region.span.longitudeDelta]] forKeys:@[@"latitude", @"longitude", @"latitudeDelta", @"longitudeDelta"]];
...
- (void)locationManager:(CLLocationManager *)manager didUpdateLocations:(NSArray *)locations {
	if(isTrackingLocation) {
		...
		CLLocationDistance distanceFromRadarLocation = [location distanceFromLocation:[[CLLocation alloc] initWithLatitude:radar.latitude longitude:radar.longitude]];
		
		if(distanceFromRadarLocation > 250) {
			radar = location.coordinate;
			[self setNewScannerLocation:location];
		}
	}
}
...
-(void)checkGPS {
    ...
    if ([CLLocationManager authorizationStatus] == kCLAuthorizationStatusNotDetermined) {
        ...
        UIAlertController *alert = [UIAlertController alertControllerWithTitle:NSLocalizedString(@"Location service denied", @"The title of an alert, that tells the user that he/she denied location access to the app.")
        ...
        UIAlertAction *ok = [UIAlertAction actionWithTitle:NSLocalizedString(@"OK", @"A common affirmative action title, like 'OK' in english.")
													 style:UIAlertActionStyleDefault
												   handler:nil];
        [alert addAction:ok];
...
-(BOOL)isPokemonVeryCommon:(NSString *)idPokeToTest {
	if(self.savedCommon == nil || [self.savedCommon count] == 0) {
		return NO;
	}
	
	NSEnumerator *enumerator = [self.savedCommon objectEnumerator];
	NSString *commonPokemonId = nil;
	
	while(commonPokemonId = [enumerator nextObject]) {
		if([idPokeToTest isEqualToString:commonPokemonId]) {
			return YES;
		}
	}
...
-(BOOL)isPokemonFavorite:(NSString *)idPokeToTest {
	if(self.savedFavorite == nil || [self.savedFavorite count] == 0) {
		return NO;
	}
	
	NSEnumerator *enumerator = [self.savedFavorite objectEnumerator];
	NSString *favoritePokemonId = nil;
	
	while(favoritePokemonId = [enumerator nextObject]) {
		if([idPokeToTest isEqualToString:favoritePokemonId]) {
			return YES;
		}
	}
	...
	if([annotation isKindOfClass:[PokemonAnnotation class]]) {
		...
		if (!view) {
            ...
            if (!view) {
                ...
                UIImage *pokestopImage = [UIImage imageNamed:@"pokestop"];
