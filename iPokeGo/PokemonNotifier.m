...
static int PokemonNotifierNotificationRangeFar = 1000;
static int PokemonNotifierNotificationRangeClose = 250;
...
-(void)displayNotificationForPokemon:(Pokemon *)pokemon
{
    ...
    if([pokemon isFav]) {
        ...
        pokemonIsInRange = distanceFromUser < PokemonNotifierNotificationRangeFar;
    } else {
        ...
        sound   = self.pokemonAppearSound;
        pokemonIsInRange = distanceFromUser < PokemonNotifierNotificationRangeClose;
    }
    
    if([prefs boolForKey:@"only_notify_in_range"] && !pokemonIsInRange) {
        NSLog(@"Not showing notification because pokemon is not in range");
        return;
    }
